...
namespace {
    ...
    void InitializeLogging() {
        ...
        *path += L".log";
        ...
        const auto& debugConfig = FreezeBodyPairs::Config::GetSingleton().GetDebug();
    ...
    void InitializeSerialization() {
        ...
        serde->SetSaveCallback(Common::HitCounterManager::OnGameSaved);
        serde->SetRevertCallback(Common::HitCounterManager::OnRevert);
        serde->SetLoadCallback(Common::HitCounterManager::OnGameLoaded);
    ...
    // * callback will not necessarily be called immediately, if the Papyrus VM has not been initialized yet (in that
    // case
    ...
    void InitializePapyrus() {
        ...
        if (GetPapyrusInterface()->Register(FreezeBodyPairs::RegisterHitCounter)) {
    ...
    void InitializeHooking() {
        ...
        FreezeBodyPairs::InitializeHook(trampoline);
    }
                    ...
                    case MessagingInterface::kInputLoaded:
                        {
                            ...
                            // 设置 Input
                            auto& input = Input::InputEventSink::GetSingleton();
                            input.SetToggleKeys(cfg.toggleKey1, cfg.toggleKey2);
                            input.SetBoneList(cfg.frozenBones);
                            input.Install();
                            break;
                        }
...
SKSEPluginLoad(const LoadInterface* skse) {
    ...
    Hook::ActorLifecycleManager::GetSingleton().Initialize();
